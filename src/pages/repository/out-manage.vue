<template>
  <div class="render">
  	<el-breadcrumb separator-class="el-icon-arrow-right">
		  ...
		  <el-breadcrumb-item class="wgTxtColor" :to="{ path: '/cataloguePage?id=1_couter'}"> 外规管理</el-breadcrumb-item>
		  <el-breadcrumb-item class="txtColor"  v-if="panelShow">{{titMsg}}</el-breadcrumb-item>
		</el-breadcrumb>
    <el-tabs v-model="activeName" @tab-click="handleClick">
    		<!--列表查询-->
	    	<el-tab-pane label="列表查询" name="first" v-if="btnData.checkList" :key="'first'">
	    			<!--查询组件-->
	    		<div class="box">
				...
				<!--全文检索-->
		    <el-tab-pane label="全文检索" v-if="btnData.textSearchBtn" style="font-weight: 600" name="second" :key="'second'">
			    		<div class="box tbox">
...
<script>
	...
	import vCheck from 'src/components/check/check'
        ...
        // 返回页面时加载数据
		   getData(aaa){
		    	...
		    	if(this.handleData !== undefined){
		    		if(this.handleData.indexOf('1_couterl')>=0){
	            		this.btnData.checkList = true
	            	}else{
	            		this.btnData.checkList = false
	            	}
			  		if(this.handleData.indexOf('1_couters')>=0){
