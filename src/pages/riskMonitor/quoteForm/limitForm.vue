<template>
  	<div id="limitForm">
		...
		<div class="quoteSys">
    		<div class="titIcon"><em></em><span style="font-weight: 600;">风险限额监测表</span></div>
    ...
    methods:{
	    ...
	    getData(handleData){
	    	if(handleData !== undefined){
            	if(handleData.indexOf('1_ifxxejcbe')>=0){//导出
            		this.btnData.exportbtn = true
            	}else{
            		this.btnData.exportbtn = false
            	}
            	if(handleData.indexOf('1_ifxxejcbd')>=0){//导出
    	...
    	// 查询列表数据
    	searchData(data){
    		this.axios.post('rmIndStass/limitIndMonrInfo',qs.stringify(data)).then((res)=>{
    			if(res.code == '200'){
    				if(res.data != null){
		        		this.isShow = true
		...
		goExport(){
			...
			this.axios.post('/rmIndStass/limitIndMonrExport',params,{responseType:'arraybuffer'}).then((res)=>{
    			let filename = "风险限额监测表.xlsx";
