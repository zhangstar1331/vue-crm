  	...
  	<div id="seasonReport">
		...
		<div class="quoteSys">
	        ...
	        <el-table :data="tableData" style="width: 100%;background: #F9F9F9!important;border: 1px solid #E9E9E9;">
	            ...
	            <el-table-column prop="name" label="检查事项" align="center" width="200">
	                <template slot-scope="scope">
	                    <span :title="scope.row.name" class="textOverFlow">{{scope.row.name == null||scope.row.name == ''?'--':scope.row.name}}</span>
	                </template>
	            </el-table-column>
	            <el-table-column prop="zt" label="实施主体" align="center">
	                <template slot-scope="scope">
...
<script>
    ...
    methods:{
	    ...
	    // 下载模板
	    goDownload(){
	    	this.axios.post('/tpl/download/701',qs.stringify(),{responseType:'arraybuffer'}).then((res)=>{
	    ...
	    uploadFile(param){
	    	...
	    	this.axios.post('/supervise/import',form).then((res)=>{
				if(res.code == '200'){
					this.searchData(this.checkData)
				}else{
					this.$alert(res.message, '提示', {
	    ...
	    goBatch(){
	    	this.axios.post('supervise/submit',JSON.stringify({flag:9}),{headers:{'Content-Type':'application/json'}}).then((res)=>{
                if(res.code == 200){
                	if(res.data !== null){
                		var tip = ""
                		if(res.data.s!==null){
