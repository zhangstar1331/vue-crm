		...
		<div class="contList firstBox">
			...
			<div class="infoDetail" v-if="active == 0">
				...
				<div class="infoItem">
					...
					<span class="infoItemC">
						<div v-if="baseinfo.base">{{baseinfo.base.endDateStr == null||baseinfo.base.endDateStr == ''?'--':baseinfo.base.endDateStr}}</div>
						<div v-else>--</div>
					</span>
				</div>
				<div class="infoItem">
	      	...
	      	<div class="infoDetail" v-if="active == 1">
				<div class="infoItem">
					...
					<span class="infoItemC">
						<div v-if="baseinfo.result">{{baseinfo.result.type == null?'--':rType[baseinfo.result.type]}}</div>
						<div v-else>--</div>
					</span>
				</div>
				<div class="infoItem">
				...
				<div class="infoItem">
					...
					<span class="infoItemC" style="vertical-align:middle">
						<el-button type="primary" size="mini" class="commonBtn" style="width:auto;padding:0 10px" @click="goEnter" :disabled="!baseinfo.penalizeId">录入信息</el-button>
					</span>
				</div>
			</div>
	      	<!-- 第三步 -->
  	...
  	export default {
	    ...
	    methods:{
	    	...
	    	goback(){
	    		if(this.$route.query.flag == 3){
					...
					if(this.companySource == "1"){
						sessionStorage.setItem('supervisePath',3)
					} else{
	    	...
	    	searchData(data){
	    		...
	    		this.axios.post('supervise/detail',qs.stringify(data)).then((res)=>{
	    			if(res.code == '200'){
	    				if(res.data!==null){
			...
			goEnter(){
	    		this.$router.push({
		    		path:'/punishDetail',
		    		query:{           //路由传参时push和query搭配使用 ，作用时传递参数
		              	infoId:this.baseinfo.penalizeId,
			            flag:1,
