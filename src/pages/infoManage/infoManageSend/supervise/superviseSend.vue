	...
	<div id="lossEventSend">
		...
		<el-tabs v-model="activeName" @tab-click="handleClick">
			<el-tab-pane v-for="(item,index) in tabListFilter" :label="item.label" :name="item.name" :key="item.name">
				<div class="box">
					<div class="titIcon"><em></em><span style="font-weight: 600;">监管检查事项信息报送查询</span></div>
					<component :key="index" :is="item.check" :isFlag="item.isFlag" v-if="activeName == item.name"></component>
				</div>
				<div class="sysBox">
					<div class="titIcon"><em></em><span style="font-weight: 600;">监管检查事项信息报送列表</span></div>
...
import vCheck from 'src/components/infoManage/superviseCheck'
import vCheckCX from 'src/components/infoManage/superviseCheckCX'
import vSysList from 'src/components/infoManage/superviseList'
import vSysListCX from 'src/components/infoManage/superviseListCX'
...
export default {
	...
	created() {
		// 专报提交返回生效库
		var path = sessionStorage.getItem('supervisePath')
		if (path == 1) {
		...
		sessionStorage.removeItem('supervisePath')
	},
	methods: {
		handleClick(tab) {
	...
	beforeRouteLeave(to, from, next) {
		let arr = ['superviseSpecialReport', 'superviseDetail','superviseSpecialReportCX', 'superviseDetailCX']
