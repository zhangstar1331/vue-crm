<template>
  	<div id="seasonReport">
		...
		<div class="quoteSys">
    		<div class="titIcon"><em></em><span style="font-weight: 600;">案件风险事件季报</span></div>
	        ...
	        <el-table :data="tableData" style="width: 100%;background: #F9F9F9!important;border: 1px solid #E9E9E9;">
	            ...
	            <el-table-column prop="caseDes" label="案件风险事件描述" align="center">
	                <template slot-scope="scope">
...
<script>
    ...
    methods:{
	    ...
	    // 下载模板
		goDownload(){
	    	this.axios.post('/tpl/download/706',qs.stringify(),{responseType:'arraybuffer'}).then((res)=>{
    			let filename = "案件风险事件季报.xlsx";
	    ...
	    uploadFile(param){
	    	...
	    	this.axios.post('/case/import',form).then((res)=>{
				if(res.code == '200'){
					this.searchData(this.checkData)
				}else{
					this.$alert(res.message, '提示', {
	    ...
	    goBatch(){
	    	this.axios.post('case/submit',JSON.stringify({flag:9}),{headers:{'Content-Type':'application/json'}}).then((res)=>{
                if(res.code == 200){
                	if(res.data !== null){
                		var tip = ""
                		if(res.data.s!==null){
