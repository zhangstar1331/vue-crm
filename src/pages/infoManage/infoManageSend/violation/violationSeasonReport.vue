<template>
  	<div id="seasonReport">
	  		...
	  		<el-row>
		  		<el-col :span="12">
			  		<el-breadcrumb separator-class="el-icon-arrow-right">
					  	...
					  	<el-breadcrumb-item @click.native="goBack" class="wgTxtColor">合规信息报送</el-breadcrumb-item>
		...
		<div class="quoteSys">
    		<div class="titIcon"><em></em><span style="font-weight: 600;">内部违规信息季报</span></div>
	        ...
	        <el-table :data="tableData" style="width: 100%;background: #F9F9F9!important;border: 1px solid #E9E9E9;">
	            ...
	            <el-table-column prop="name" label="违规事由" align="center" width="200">
...
<script>
    ...
    methods:{
	    ...
	    // 下载模板
		goDownload(){
	    	this.axios.post('/tpl/download/704',qs.stringify(),{responseType:'arraybuffer'}).then((res)=>{
    			let filename = "内部违规信息季报.xlsx";
	    ...
	    uploadFile(param){
	    	...
	    	this.axios.post('/violate/import',form).then((res)=>{
				if(res.code == '200'){
					this.searchData(this.checkData)
				}else{
					this.$alert(res.message, '提示', {
	    ...
	    goBatch(){
	    	this.axios.post('violate/submit',JSON.stringify({flag:9}),{headers:{'Content-Type':'application/json'}}).then((res)=>{
                if(res.code == 200){
                	if(res.data !== null){
                		var tip = ""
                		if(res.data.s!==null){
