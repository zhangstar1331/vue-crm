<template>
  	<el-container id="violationDetail">
  		<div id="top"></div>
		...
		<div class="contList firstBox">
			...
			<div class="infoDetail" v-if="active == 0">
				<div class="infoItem" v-if="this.$route.query.flag == 1">
					<span class="infoItemT">内部违规事件编号：</span>
					<span class="infoItemC">{{baseinfo.sn == null?'--':baseinfo.sn}}</span>
				</div>
				<div class="infoItem">
					<span class="infoItemT">违规事由：</span>
				...
				<div class="infoItem">
					<span class="infoItemT">违规主体：</span>
					<span class="infoItemC">
						<div v-if="baseinfo.base">{{baseinfo.base.organizationStr == null||baseinfo.base.organizationStr == ''?'--':baseinfo.base.organizationStr}}</div>
						<div v-else>--</div>
					</span>
				</div>
				<div class="infoItem">
					<span class="infoItemT">事件发现时间：</span>
					<span class="infoItemC">
	      	...
	      	<div class="infoDetail" v-if="active == 1">
				...
				<div class="infoItem">
					...
					<span class="infoItemC">
						<div v-if="baseinfo.abarbeitung">{{baseinfo.abarbeitung.description == null||baseinfo.abarbeitung.description == ''?'--':baseinfo.abarbeitung.description}}</div>
						<div v-else>--</div>
					</span>
				</div>
				<div class="infoItem">
	      	...
	      	<div class="infoDetail" v-if="active == 2">
				...
				<div class="infoItem">
					...
					<span class="infoItemC">
						<div v-if="baseinfo.duty">{{baseinfo.duty.description == null||baseinfo.duty.description == ''?'--':baseinfo.duty.description}}</div>
						<div v-else>--</div>
					</span>
				</div>
				<div class="infoItem">
		...
		<div class="contList" v-if="active == 0">
			<div class="infoDetail" v-if="active == 0">
				<div class="titIcon fontWeight" style="text-align:left"><em></em><span style="font-weight: 600;">造成损失</span></div>
				<div class="infoItem">
					<span class="infoItemT">财产损失（万元）：</span>
					<span class="infoItemC">
						<div v-if="baseinfo.base">{{baseinfo.base.loss == null||baseinfo.base.loss == ''?'--':baseinfo.base.loss}}</div>
						<div v-else>--</div>
					</span>
				</div>
				<div class="infoItem">
					<span class="infoItemT">社会影响：</span>
					<span class="infoItemC">
						<div v-if="baseinfo.base">{{baseinfo.base.affect == null||baseinfo.base.affect == ''?'--':baseinfo.base.affect}}</div>
						<div v-else>--</div>
					</span>
				</div>
				<div class="infoItem">
					<span class="infoItemT">其他：</span>
					<span class="infoItemC">
						<div v-if="baseinfo.base">{{baseinfo.base.other == null||baseinfo.base.other == ''?'--':baseinfo.base.other}}</div>
						<div v-else>--</div>
					</span>
				</div>
				<div class="infoItem">
					<span class="infoItemT" style="vertical-align:middle">附件：</span>
					<span class="infoItemC" style="vertical-align:middle">
						<el-button type="primary" size="mini" class="commonBtn" @click="loadFile(baseinfo.base.attachmentLoss)" :disabled="!baseinfo.base||!baseinfo.base.attachmentLoss">批量下载</el-button>
						<i class="el-icon-paperclip"></i>共{{baseinfo.base&&baseinfo.base.attachmentLoss?baseinfo.base.attachmentLoss.length:'0'}}个附件
					</span>
				</div>
				<div class="infoItem">
					<span v-if="baseinfo.base&&baseinfo.base.attachmentLoss" class="file">
						<el-popover placement="top" trigger="click" v-for="(item,index) in baseinfo.base.attachmentLoss" :key="item.id" :disabled="item.url.substring(item.url.lastIndexOf('.')+1) == 'rar'||item.url.substring(item.url.lastIndexOf('.')+1) == 'zip'?true:false" v-model="item.visible">
							<div style="text-align:center;margin:0">
								<el-button type="primary" size="mini" class="commonBtn" @click="preView(item.url,item.name,(item.visible = !item.visible))">预览</el-button>
								<el-button type="primary" size="mini" class="commonBtn" @click="goDownload(item.url,item.name,1)">下载</el-button>
							</div>
							<em @click="goDownload(item.url,item.name)" slot="reference">
							{{item.name == null?'--':item.name+item.url.substring(item.url.lastIndexOf('.'))}}<i style="color:#000" v-if="index !== baseinfo.base.attachmentLoss.length-1">、</i><i style="color:#000" v-else>。</i>
							</em>
						</el-popover>
					</span>
					<span v-else>--</span>
		...
		<div class="contList">
			...
			<el-table :data="tableData" style="width: 100%;background: #F9F9F9!important;border: 1px solid #E9E9E9;">
				<el-table-column prop="version" label="内部违规信息版本号" align="center" width="180">
	                <template slot-scope="scope">
	                    <span :title="scope.row.version" class="textOverFlow">{{scope.row.version == null||scope.row.version == ''?'--':scope.row.version}}</span>
	                </template>
	            </el-table-column>
				<el-table-column prop="name" label="违规事由" align="center">
	                <template slot-scope="scope">
            	...
            	<el-table-column prop="submitDate" label="报送时间" align="center">
	                <template slot-scope="scope">
	                    <span :title="scope.row.submitDate" class="textOverFlow">{{scope.row.submitDate == null||scope.row.submitDate == ''?'--':scope.row.submitDate}}</span>
	                </template>
	            </el-table-column>
	            <el-table-column prop="upgradeDate" label="维护时间" align="center">
	                <template slot-scope="scope">
	                    <span :title="scope.row.upgradeDate" class="textOverFlow">{{scope.row.upgradeDate == null||scope.row.upgradeDate == ''?'--':scope.row.upgradeDate}}</span>
	                </template>
	            </el-table-column>
			</el-table>
		</div>
	...
	import qs from 'qs'
  	export default {
	    ...
	    methods:{
	    	...
	    	goback(){
	    		if(this.$route.query.flag == 3){
	    			...
	    			sessionStorage.setItem('statisPath',3)
	    			this.$router.push({
	    			...
	    			sessionStorage.setItem('sendBackPath','fourth')
	    			this.$router.push({
			    		name: 'sendBack',
			    		query:{           //路由传参时push和query搭配使用 ，作用时传递参数
              				id: this.$route.query.id
	    	...
	    	searchData(data){
	    		...
	    		this.axios.post('violate/detail',qs.stringify(data)).then((res)=>{
	    			if(res.code == '200'){
	    				if(res.data!==null){
	    	...
	    	getHistory(data){
	    		delete data.flag
	    		this.axios.post('violate/history',qs.stringify(data)).then((res)=>{
	    			if(res.code == '200'){
	    				if(res.data!==null){
	    					this.tableData = []
		    ...
		    goDownload(url,name,type){
		    	...
		    	if(fileType == 'rar' || fileType == 'zip' || type == '1'){
					...
					this.axios.post('comm/download/704',qs.stringify(obj),{responseType:'arraybuffer'}).then((res)=>{
	    ...
	    beforeRouteEnter (to, from, next) {
			next((that) => {
			    if(from.fullPath.includes('/violationInfo')){
			    	that.path = true
			    }
