  	...
  	<div id="lossEventSend">
		...
		<el-tabs v-model="activeName" @tab-click="handleClick">
			<el-tab-pane label="草稿箱" name="first" :key="'first'" v-if="tab1">
				<div class="box">
					<div class="titIcon"><em></em><span style="font-weight: 600;">问责信息报送查询</span></div>
					<v-check :isFlag = '2' v-if="isChildUpdate1"></v-check>
				</div>
				<div class="sysBox">
					<div class="titIcon"><em></em><span style="font-weight: 600;">问责信息报送列表</span></div>
...
<script>
    ...
    created(){
    	// 专报提交返回生效库
    	var path = sessionStorage.getItem('accountPath')
    	if(path == 1){
    	...
    	sessionStorage.removeItem('accountPath')
    },
    methods:{
    	handleClick(tab) {
	       if(tab.name == "first") {
  	...
  	beforeRouteLeave(to,from,next){
	    let arr = ['accountSpecialReport','accountDetail']
