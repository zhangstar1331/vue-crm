<template>
	<el-container id="reEdit" class="container">
			...
			<el-row>
				<el-col :span="12">
					<el-breadcrumb separator-class="el-icon-arrow-right">
						...
						<el-breadcrumb-item :to="{ path: '/repository/white-manage',query:{id:'1_cdw'}}">{{this.companySource == '1'?'制度库（编辑中）':'待维护制度库'}}</el-breadcrumb-item>
						<el-breadcrumb-item class="txtColor">{{wgName}}</el-breadcrumb-item>
					</el-breadcrumb>
				</el-col>
				<el-col :span="12" style="text-align: right;">
					<el-button type="primary" class="standardBtn" @click="back" size="mini">返回</el-button>
		...
		<div class="render">
			<div class="box" style="margin-top: 55px;min-height: 250px;">
		...
		<div class="render mbtm20">
			<div class="box" style="padding-bottom: 16px;">
				...
				<el-row>
					...
					<el-col :span="12">
						<el-upload class="upload-demo" style="text-align: right;" :show-file-list="false" ref="upload" action="" :http-request="uploadFile" multiple accept=".doc,.docx,.pdf,.Excel,.xls,.xlsx">
		...
		<!--拆分导入-->
		<div class="section-rowdccs" id="reEdits" v-if="tableShow">
			<div class="section-dealise">
				<div class="titIcon">
					...
					<el-button type="primary" class="standardBtn" v-if="downLoadBtn" style="margin-right: 0;" @click="downLoadTemplate">下载模板</el-button>
...
<script>
import regulationsCheck from 'src/components/check/regulationsCheck'
import maskTable from 'src/components/maskTable/maskTable'
...
export default {
	...
	created() {
		this.getReviseId()
		sessionStorage.saveDatas = JSON.stringify({ 'type': '2' });
	},
	computed: {
	...
	methods: {
		...
		getDetail(id) {
			this.axios.post('instweb/detail', JSON.stringify({ id: id, key: '2' }), { headers: { 'Content-Type': 'application/json' } }).then((res) => {
				...
				if (res.data.file != undefined) {
					...
					for (var i = 0; i < res.data.file.length; i++) {
						...
						if (res.data.file[i].name.indexOf('附件') != -1 || res.data.file[i].type == '0') {
		...
		//查询法条数据
		checkData(data) {
			this.axios.post('instweb/item/query', JSON.stringify(data), { headers: { 'Content-Type': 'application/json' } }).then((res) => {
		...
		deleteFile(index, row) {
			this.$confirm('确认删除？', '提示', {
				...
				this.axios.post('instweb/delFile', JSON.stringify(this.deleteFileData), { headers: { 'Content-Type': 'application/json' } }).then((res) => {
					const infoData = JSON.parse(localStorage.getItem('infoData'));
					const saveDatas = JSON.parse(sessionStorage.getItem('saveDatas'))
		...
		handleDelete(index, row) {
			this.$confirm('确认删除？', '提示', {
				...
				this.axios.post('instweb/item/del', JSON.stringify(this.deleteSingleData), { headers: { 'Content-Type': 'application/json' } }).then((res) => {
					this.splitData.institutionId = this.infoData.institutionId;
					this.splitData.reviseId = this.infoData.reviseId;
					this.splitData.fileId = fileID;
					if (res.code === 200) {
		...
		deleteAll() {
			this.$confirm('确认删除？', '提示', {
				...
				this.axios.post('instweb/item/delAll', JSON.stringify(this.deleteAllData), { headers: { 'Content-Type': 'application/json' } }).then((res) => {
					if (res.code == 200) {
						this.getInfoData();
						this.deleteAllData.institutionId = this.infoData.institutionId;
						this.deleteAllData.fileId = fileID;
		...
		saveAll() {
			...
			if (!(saveDatas.fileNoHead != undefined && saveDatas.fileNoHead != '' && saveDatas.fileNoBody != '' && saveDatas.fileNoFoot != '')) {
				...
				return;
			} else if (saveDatas.category == undefined || saveDatas.category == '') {
				this.$alert('制度类别不能为空', '提示', {
					confirmButtonText: '确定'
				});
				return;
			} else if (saveDatas.publish == undefined || saveDatas.publish == '') {
				this.$alert('发文机关不能为空', '提示', {
					confirmButtonText: '确定'
				});
				return;
			...
			this.axios.post('instweb/saveBase', JSON.stringify(saveDatas), { headers: { 'Content-Type': 'application/json' } }).then((res) => {
				if (res.code == 200) {
					this.$alert(res.message, '提示', {
						confirmButtonText: '确定'
					});
		...
		submitAll() {
			const saveDatas = JSON.parse(sessionStorage.getItem('saveDatas'))
			saveDatas.range = saveDatas.range == undefined ? null : saveDatas.range.toString();
			//校验基本信息是否为空
			if (!(saveDatas.fileNoHead != undefined && saveDatas.fileNoHead != '' && saveDatas.fileNoBody != '' && saveDatas.fileNoFoot != '')) {
				this.$alert('制度文号不能为空', '提示', {
					confirmButtonText: '确定'
			...
			this.axios.post('instweb/submit', JSON.stringify(saveDatas), { headers: { 'Content-Type': 'application/json' } }).then((res) => {
				if (res.code == 200) {
					this.$alert('提交成功', '提示', {
						confirmButtonText: '确定',
						callback: action => {
							this.$router.push({
								path: '/repository/white-manage', //跳转的路径
								query: { id: '1_cdw' }
							})
						}
					});
...
<style lang="scss">
#reEdits .el-table__body-wrapper /deep/ tr td:nth-child(3)>.cell {
	white-space: pre-line !important;
	text-indent: 0 !important;
	text-align: left !important;
	padding-left: 5%;
}
#reEdits .el-table__body-wrapper /deep/ tr td {
	text-align: center;
	text-indent: 0;
	border-right: none !important;
}
#reEdits .el-table__header /deep/ th {
	border-right: none !important;
}
#reEdits .el-table__header /deep/ th>.cell {
	text-align: center;
	float: left;
	text-indent: 0;
}
