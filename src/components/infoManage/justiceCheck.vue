			        	...
			        	<el-row>
			                ...
			                <el-col :span="isNotice == 1?12:8">
			                    <el-form-item label="案件专报时间">
								    <el-date-picker v-model="lossSendForm.reportDate" @change="changeReportDate" placeholder="开始时间" :picker-options="starDatePicker1"></el-date-picker>
								</el-form-item>
			                </el-col>
			                <el-col :span="isNotice == 1?12:8">
			                    <el-form-item label="案件发生机构" class="searchBtn">
								    <el-select v-model="mineStatus" multiple collapse-tags placeholder="请选择案件发生机构" ref="searchSelect" value-key="mineStatus" @remove-tag="deleteOrg" filterable :filter-method="keyFilter" @focus="selectFoucus" @visible-change="visibleChange">
			            	...
			            	<el-col :span="isNotice == 1?12:8">
			                    <el-form-item label="案件名称">
								    <el-input v-model="lossSendForm.name" @change='getName' placeholder="请输入案件名称" prefix-icon="el-icon-search"></el-input>
								</el-form-item>
			                </el-col>
			                <el-col :span="isNotice == 1?12:8">
			                    <el-form-item label="是否问责">
			                    	<el-select v-model="isWz.region" @change="changeWz" placeholder="请选择实施主体">
								    	<el-option v-for="item in isWz.unitsData" :key="item.id" :label="item.name" :value="item.id"></el-option>
								    </el-select>
								</el-form-item>
			                </el-col>
			                <el-col :span="isNotice == 1?12:8" v-if="this.isSend == 1">
			                    <el-form-item label="案件类型">
			                    	<el-select v-model="justiceType.region" @change="changejusticeType" placeholder="请选择实施主体">
...
<script>
	...
	import qs from 'qs'
	export default {
		props:['isFlag','isStore','checkList','isJusticeType','isSend','isNotice'],
	    data () {
	      return {
	      	lossSendForm:{
	      		name:'',//检查事项名称
	    ...
	    mounted () {
    		...
    		if(this.isJusticeType){
    			this.checkData.justiceType = this.isJusticeType
    		}
	    	...
	    	if(JSON.parse(sessionStorage.getItem('askData'+this.isFlag))){
	    		...
	    		if(this.checkData.isWz == '0'){
	    			this.isWz.region = '否'
	    		} else if (this.checkData.isWz == '1'){
	    			this.isWz.region = '是'
	    		} else{
	    			this.isWz.region = '全部'
	    		}
	    		if(this.checkData.justiceType){
	    			this.justiceType.region = this.checkData.justiceType
	    		}
	    	}
	    	// 获取表格数据
	    ...
	    methods: {
	    	...
	    	getName(val){
	      	...
	      	// 实施主体
	      	changeWz(val){
	      		this.checkData.isWz = val
	      	},
	      	changejusticeType(val){
	      		this.checkData.justiceType = val
	      	},
	      	//被检查机构
				...
				this.$set(this.checkData,'mineStatus',this.mineStatus)
	      		...
	      		if(this.$route.query.id == '1_hsfajxxbs'){
					if(this.isFlag == 1){
						...
						if(this.isJusticeType == 1){
							this.$emit('changeSessionStorage',{no:2,data:this.checkData})
						} else{
