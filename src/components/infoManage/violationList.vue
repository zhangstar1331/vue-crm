<template>
    <div id="lossSendList">
        ...
        <el-table :data="tableData4" style="width: 100%;background: #F9F9F9!important;border: 1px solid #E9E9E9;">
            ...
            <el-table-column prop="sn" label="内部违规事件编号" align="center" width="150" v-if="this.isFlag == 1 || this.isSend == 1">
                <template slot-scope="scope">
                    <span :title="scope.row.sn" class="textOverFlow">{{scope.row.sn == null||scope.row.sn == ''?'--':scope.row.sn}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="name" label="违规事由" align="center" width="200" v-if="this.isFlag == 2">
                <template slot-scope="scope">
                    <span :title="scope.row.name" class="textOverFlow">{{scope.row.name == null||scope.row.name == ''?'--':scope.row.name}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="name" label="违规事由" align="center" width="200" v-if="this.isFlag == 1&&!this.isStore&&!this.isSend">
                <template slot-scope="scope">
            ...
            <el-table-column prop="name" label="违规事由" align="center" width="200" v-if="this.isStore == 1">
                <template slot-scope="scope">
            ...
            <el-table-column prop="name" label="违规事由" align="center" width="200" v-if="this.isSend == 1">
                <template slot-scope="scope">
            ...
            <el-table-column prop="occurDate" label="事件发现时间" align="center">
                <template slot-scope="scope">
                    <span>{{scope.row.occurDate == null||scope.row.occurDate == ''?'--':scope.row.occurDate}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="org" label="违规主体" align="center">
                <template slot-scope="scope">
                    <span :title="scope.row.org" class="textOverFlow">{{scope.row.org == null||scope.row.org == ''?'--':scope.row.org}}</span>
                </template>
            </el-table-column>
            <el-table-column prop="fourthOrganization" label="四级机构" align="center">
                <template slot-scope="scope">
                    <span :title="scope.row.fourthOrganization" class="textOverFlow">{{scope.row.fourthOrganization == null||scope.row.fourthOrganization == ''?'--':scope.row.fourthOrganization}}</span>
                </template>
            </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      goSpecialReport(){
          this.$router.push({
            path:'/violationSpecialReport',   //跳转的路径
            query:{           //路由传参时push和query搭配使用 ，作用时传递参数
              id: this.$route.query.id
            }
          })
      ...
      refresh(){
         ...
         this.axios.post('violate/query',JSON.stringify(askData),{headers:{'Content-Type':'application/json'}}).then((res)=>{
            if(res.code == 200){
              if(res.data != null){
      ...
      goRollback(index,row){
        this.$prompt('请输入退回原因', '提示', {
            ...
            this.axios.post('violate/rollback',qs.stringify({id:row.id,msg:value})).then((res)=>{
            if(res.code == 200){
                this.$alert('退回成功', '提示', {
                    confirmButtonText: '确定',
                    callback:action=>{
      ...
      // 提交
      goSubmit(index,row){
          this.axios.post('violate/submit',JSON.stringify({id:row.id,flag:0}),{headers:{'Content-Type':'application/json'}}).then((res)=>{
            if(res.code == 200){
              if(res.data == 0){
      ...
      goDelete(index,row){
          this.$confirm('确认删除？', '提示', {
              ...
              this.axios.post('violate/del',qs.stringify({id:row.id})).then((res)=>{
                if(res.code == 200){
                  this.$alert('删除成功', '提示', {
                      confirmButtonText: '确定',
                      callback:action=>{
