			        	...
			        	<el-row>
			                ...
			                <el-col :span="isNotice == 1?12:8">
			                    <el-form-item label="检查事项名称">
								    <el-input v-model="lossSendForm.thingName" @change='getthingName' placeholder="请输入检查事项名称" prefix-icon="el-icon-search"></el-input>
								</el-form-item>
			                </el-col>
			                <el-col :span="isNotice == 1?12:8">
	...
	export default {
	    ...
	    mounted () {
	    	...
	    	if(JSON.parse(sessionStorage.getItem('askData'+this.isFlag))){
	    		...
	    		if(this.checkData.thingName){
	    			this.lossSendForm.thingName = this.checkData.thingName
	    		}
	    		if(this.checkData.subjectType){
	    			this.subject.region = this.checkData.subjectType
	    		}
	    ...
	    methods: {
	    	...
	    	getthingName(val){
	      		if(val == ''){
	      			delete this.checkData.thingName
	      		} else{
	      			this.checkData.thingName = val
	      		}
	      	},
	      	// 实施主体
	      	changeSubject(val){
