...
<div id="lossSendList">
    ...
    <el-table :data="tableData4" style="width: 100%;background: #F9F9F9!important;border: 1px solid #E9E9E9;">
        ...
        <el-table-column prop="sn" label="行政处罚事先告知书信息编号" align="center" width="180" v-if="this.isFlag == 1||this.isSend == 1">
            <template slot-scope="scope">
                <span :title="scope.row.sn" class="textOverFlow">{{scope.row.sn == null||scope.row.sn == ''?'--':scope.row.sn}}</span>
            </template>
        </el-table-column>
        <el-table-column prop="punishType" label="拟处罚类型" align="center" width="150">
            <template slot-scope="scope">
                <span>{{scope.row.punishType == null||scope.row.punishType == ''?'--':puninshType[scope.row.punishType]}}</span>
            </template>
        </el-table-column>
        <el-table-column prop="overView" label="拟处罚概述" align="center" v-if="this.isFlag == 2">
            <template slot-scope="scope">
                <span :title="scope.row.overView" class="textOverFlow">{{scope.row.overView == null||scope.row.overView == ''?'--':scope.row.overView}}</span>
            </template>
        </el-table-column>
        <el-table-column prop="overView" label="拟处罚概述" align="center" v-if="this.isFlag == 1&&!this.isStore&&!this.isSend">
            <template slot-scope="scope">
        ...
        <el-table-column prop="overView" label="拟处罚概述" align="center" v-if="this.isStore == 1">
            <template slot-scope="scope">
        ...
        <el-table-column prop="cause" label="拟处罚事由" align="center" width="200" v-if="this.isSend == 1">
            <template slot-scope="scope">
                <span :title="scope.row.cause" class="textOverFlow" style="color:#4285F4" @click="goDetail(scope.$index, scope.row)">{{scope.row.cause == null||scope.row.cause == ''?'--':scope.row.cause}}</span>
            </template>
        </el-table-column>
        ...
        <el-table-column prop="orgName" label="拟被处罚机构" align="center" v-if="this.isSend == 1">
            <template slot-scope="scope">
                <span :title="scope.row.orgName" class="textOverFlow">{{scope.row.orgName == null||scope.row.orgName == ''?'--':scope.row.orgName}}</span>
            </template>
        </el-table-column>
        <el-table-column prop="lkDate" label="行政处罚事先告知书落款时间" align="center" width="180">
            <template slot-scope="scope">
                <span>{{scope.row.lkDate == null||scope.row.lkDate == ''?'--':timeFormat(scope.row.lkDate)}}</span>
            </template>
        </el-table-column>
        <el-table-column prop="submitDate" label="报送时间" align="center" width="120" v-if="this.isFlag == 1||this.isSend == 1">
            <template slot-scope="scope">
        ...
        <el-table-column prop="bs,rs,ds,as" label="操作" align="center" width="300" v-if="this.isFlag == 2">
            <template slot-scope="scope">
        ...
        <el-table-column prop="bs,rs,ds,as" label="操作" align="center" width="200" v-if="this.isFlag == 1&&!this.isStore&&!this.isSend">
            <template slot-scope="scope">
                <span class="handleBtn detailBtn" @click="goDetail(scope.$index, scope.row)" style="color: #4285F4;" v-if="btnData.detailbtn">详情</span>
                <span class="handleBtn detailBtn" @click="goRollback(scope.$index, scope.row)" style="color: #4285F4;" v-if="btnData.rollbackbtn==1&&scope.row.rollback==1">退回</span>
    ...
    <hg-mark :delay="delay" url="cx_penalizeNotice/updateMark" v-if="isShow" @closeMask="closeMask"></hg-mark>
</div>
</template>
...
export default {
    ...
    methods: {
        ...
        goSeasonReport() {
            this.$router.push({
                path: '/punishNoticeSeasonReport', //跳转的路径
        ...
        goSpecialReport() {
            this.$router.push({
                path: '/punishNoticeSpecialReport', //跳转的路径
                query: { //路由传参时push和query搭配使用 ，作用时传递参数
                    id: this.$route.query.id
                }
            })
        ...
        refresh() {
            ...
            this.axios.post('cx_penalizeNotice/query', JSON.stringify(askData), {
                headers: {
        ...
        goRollback(index, row) {
            this.$prompt('请输入退回原因', '提示', {
                ...
                this.axios.post('cx_penalizeNotice/rollback', qs.stringify({
                    id: row.id,
                    msg: value,
                    status: this.isFlag
                })).then((res) => {
        ...
        goSubmit(index, row, type) {
            this.axios.post('cx_penalizeNotice/submit', JSON.stringify({
                id: row.id,
                status: 1
            }), {
                headers: {
        ...
        goDelete(index, row) {
            this.$confirm('确认删除？', '提示', {
                    ...
                    this.axios.post('cx_penalizeNotice/delete', qs.stringify({
                        id: row.id
                    })).then((res) => {
                        if (res.code == 200) {
                            this.$alert('删除成功', '提示', {
        ...
        // 标记
        goMark(index, row) {
            this.axios.post('cx_penalizeNotice/queryMark', qs.stringify({
                id: row.id
            })).then((res) => {
                if (res.code == 200) {
                    this.isShow = true
