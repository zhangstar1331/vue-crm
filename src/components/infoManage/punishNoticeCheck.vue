			        ...
			        <el-form :model="lossSendForm" label-position="left" ref="lossSendForm" size="mini">
			        	<el-row>
			                <el-col :span="isNotice == 1?12:8">
			                    <el-form-item label="拟处罚事由名称">
								    <el-input v-model="lossSendForm.cause" @change='getCause' placeholder="请输入拟处罚事由名称" prefix-icon="el-icon-search"></el-input>
								</el-form-item>
			                </el-col>
			                <el-col :span="isNotice == 1?12:8">
			                ...
			                <el-col :span="isNotice == 1?12:8">
								<orgTreeMulti label="拟被处罚机构" ref="orgTreeCX" @orgTreeChange="orgTreeChange" :isFlag="isFlag"></orgTreeMulti>
			                </el-col>
			            </el-row>
			        </el-form>
			    </div>
	    ...
	    methods: {
			...
			searchData (data) {
				...
				if(data.org&&data.org.indexOf("1_01")>=0){
	      		...
	      		// 获取表格数据
	      		this.axios.post('cx_penalizeNotice/query',JSON.stringify(data),{headers:{'Content-Type':'application/json'}}).then((res)=>{
		            if(res.code == 200){
		            	if(res.data != null){
