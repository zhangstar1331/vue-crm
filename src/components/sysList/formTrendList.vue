<template>
    <div id="formTrendList">
        <div v-if="isShow" style="height:100%">
            <el-table :data="tableData4" style="width: 100%;background: #F9F9F9!important;border: 1px solid #E9E9E9;" height="80%" ref="tableRef">
                ...
                <el-table-column v-for="(item,index) in quoteMonth" prop="" :key="index" :label="item" align="center" width="200">
                    <template slot-scope="scope">
                      <div v-if="scope.row['t'+quoteMonth[index]]!==undefined">
                          <div :class="{'redBg':scope.row['t'+quoteMonth[index]].rybStatus == 1,'yellowBg':scope.row['t'+quoteMonth[index]].rybStatus == 2,'blueBg':scope.row['t'+quoteMonth[index]].rybStatus == 3}">
                            <p v-for="item in scope.row['t'+quoteMonth[index]].value" :title="item">{{item}}</p >
                        </div>
                      </div>
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="caleName,defins" label="指标定义及口径" width="250">
                    <template slot-scope="scope">
                ...
                <el-table-column prop="isryb" label="本期是否评价类" align="center" width="120" v-if="limitYear&&limitYear<2020">
                    <template slot-scope="scope">
                ...
                <el-table-column prop="revision" label="指标修订记录" width="250">
                    <template slot-scope="scope">
                      <div v-for="item in scope.row.revision" class="textOverFlow" style="text-align:left" v-if="scope.row.revision!==null||scope.row.revision!==''">
  ...
  export default {
    ...
    created(){
      ...
      Bus.$on('dataList', (dataList) => {
        this.isShow = true
        this.totalResults = dataList.data.total == undefined?1:dataList.data.total
        //处理月份数据
        ...
        this.tableData4 = []
        this.tableData4 = dataList.data.instats == undefined?[]:dataList.data.instats
        this.$nextTick(()=>{
    ...
    methods: {
      ...
      getData(handleData){
        if(handleData !== undefined){
          if(handleData.indexOf('1_ifxzbcxqd')>=0){
      ...
      refresh(){
          ...
          this.axios.post('rmIndStass/indDriftReportInfo',qs.stringify(askData)).then((res)=>{
              if(res.code == 200){
                  if(res.data != null){
                      if(res.data.instats != undefined || res.data.instats != null){
                        for (var i = 0; i < res.data.instats.length; i++) {
                          for (var j = 0; j < this.monthLen+1; j++) {
      ...
      goDetail(index,row){
        ...
        this.$router.push({
          ...
          query:{           //路由传参时push和query搭配使用 ，作用时传递参数
            indId:row.id,
            subDateBegin:askData.subDateBegin,
            subDateEnd:askData.subDateEnd,
            isTrend:1,
