<template>
	<div class="mask" v-if="faShow" style="text-indent: 26px;" >
		...
		<div class="maskBox" style="height: 369px;overflow: auto;">
		  	<el-table :data="tableData" style="background: #F9F9F9!important;border: 1px solid #E9E9E9;">
			    ...
			    <el-table-column prop="name" label="制度内容" >
			    	<template slot-scope="scope">
				      <div class="input-box">
				         <el-input size="small" type="textarea" autosize v-if="allBtn" @blur="handleInputBlur(scope.$index,scope.row)" show-overflow-tooltip v-model="scope.row.contentTxt" ></el-input>
				         <el-input size="small" type="textarea" autosize v-if="singleBtn" @blur="singleBlur(scope.$index,scope.row)" show-overflow-tooltip v-model="tableData[0].content" ></el-input>
				      </div>
				   </template>
			    </el-table-column>
		  	</el-table>
