			  ...
			  <el-col :span="18">
			  	<el-tree :data="datas" ref='tree2' :props="defaultProps" node-key="id" @node-expand="getNodeInfo" :default-expanded-keys="arrId" :expand-on-click-node="false"  @node-click="handleNodeClick">
			  		<span slot-scope="{node,data}">
						{{node.label}}<span v-if="data.id== 1 || data.id == 2 || data.id == 3">（{{data.count}}）</span>
					</span>
			  	</el-tree>
			  </el-col>
			</el-row>
	...
	export default {
	    ...
	    created(){
	    	if(this.companyData != null){
		    	let cont1 = this.companyData['all_1'] == undefined || null || ''? '0' : this.companyData['all_1']
	    		let cont2 = this.companyData['all_2'] == undefined || null || ''? '0' : this.companyData['all_2']
	    		let cont3 = this.companyData['all_3'] == undefined || null || ''? '0' : this.companyData['all_3']
	    		this.datas[0].count = cont1
	    		this.datas[1].count = cont2
	    		this.datas[2].count = cont3
	    ...
	    methods:{
	    	getData(tab, event){
	    		...
	    		var cont1,
	    			...
	    			cont3
	    		if(this.companyData != null){
	    			cont1 = this.companyData[tabName+'_1'] == undefined || null || ''? '0' : this.companyData[tabName+'_1']
	    			cont2 = this.companyData[tabName+'_2'] == undefined || null || ''? '0' : this.companyData[tabName+'_2']
	    			cont3 = this.companyData[tabName+'_3'] == undefined || null || ''? '0' : this.companyData[tabName+'_3']
	    		}else{
	    			cont1 = 0;
	    			cont2 = 0;
	    			cont3 = 0;
	    	...
	    	getTreeData(paramsId){
	    		...
	    		this.axios.post('instweb/query/mtree',JSON.stringify(paramsId),{headers:{'Content-Type':'application/json'}}).then((res)=>{
    				...
    				this.datas[index-1].children = arr
    				if(this.num < 4){
		    			this.askData.manageLevel = this.num++;
		    			this.getTreeData(this.askData)
		    		}
	    		})
	    	}
